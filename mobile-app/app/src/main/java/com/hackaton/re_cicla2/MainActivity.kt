...
fun MainApp(authViewModel: AuthViewModel, googleSignInLauncher: androidx.activity.result.ActivityResultLauncher<Intent>) {
    ...
    // Determinamos el destino inicial basándonos en si el usuario ya está autenticado
    val startDestination = remember {
        if (authViewModel.isUserLoggedIn()) "home" else "login"
    }

    // Escuchar cambios en el estado para forzar la navegación al login si se cierra sesión
    ...
    NavHost(navController = navController, startDestination = startDestination) {
        ...
        composable("home") {
            ...
            HomeScreen(
                ...
                userCode = successState?.userCode ?: "",
                points = successState?.points ?: 0,
                transactions = successState?.transactions ?: emptyList(),
                viewModel = authViewModel,
                onSignOut = {
                    // El cierre de sesión ya limpia el estado y el LaunchedEffect navegará
                },
